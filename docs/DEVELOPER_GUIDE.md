...
# 📄 Part 1: File Generation Standards

> **Purpose:** Ensure ALL generated files have minimum executable content — no empty files!

---

## 🚫 CRITICAL RULE: NO EMPTY FILES

**⚠️ All generated files MUST be immediately compilable and executable!**

Every file generated from prompts MUST contain:

- ✅ Proper package/namespace declarations
- ✅ Required imports
- ✅ Actual implementations (not just signatures/placeholders)
- ✅ Minimum line count thresholds
- ✅ Compilable code (mvn compile must pass)

---

## 📊 Minimum Requirements by File Type
...
public class [PageName] extends BasePage {
    ...
    public void enter[Field](String value) {
        log.info("📝 Entering value in [field]: " + value);
    ...
    public boolean verify[Element]Visible() {
        ...
        log.info(isVisible ? "✅ [Element] is visible" : "❌ [Element] not visible");
        return isVisible;
    }
}
```
...
public class [Feature]Steps extends base {
    ...
    public void verificationStep() {
        log.info("📍 Step: Additional verification");
...
```
1. Input Validation:
   ✓ Recording has >= 3 actions
   ✓ JIRA story has >= 1 acceptance criterion
   ✓ Feature name is valid Java identifier
   ✓ All required fields populated

2. If Insufficient Input:
   → Recording < 3 actions: Generate 5 common actions
   → JIRA no ACs: Generate 3 generic CRUD scenarios
   → Page no elements: Generate common elements
   → Steps undefined: Generate implementations calling page methods
```
...
1. Line Count Validation:
   ...
   ✓ Feature File >= 15 lines
   ✓ Step Definitions >= 60 lines
   ✓ Configuration >= 10 properties

2. Keyword Validation:
   ✓ Page Object contains: "extends BasePage", "public void", "private Locator"
   ✓ Feature contains: "Feature:", "Scenario:", "Given", "When", "Then"
   ✓ Steps contain: "@Given", "@When", "@Then", "public void"
...
4. Implementation Validation:
   ✓ No placeholder comments left: [TODO], [FIXME] without code
   ✓ All methods have body (not just signatures)
...
|-----------|-----------|-------------|-------------|--------------|
| **Page Object** | 50 | 3 | BasePage | ✅ Yes |
| **Feature File** | 15 | N/A (3 steps) | N/A | ✅ Yes (Gherkin) |
| **Step Definitions** | 60 | 3 steps | base | ✅ Yes |
| **Configuration** | 10 props | N/A | N/A | ✅ Yes |
| **Test Runner** | 40 | 1 | AbstractTestNGCucumberTests | ✅ Yes |

### Quality Gates

```
✅ PASS: all files >= minimum line count · mvn compile SUCCESS · no empty methods
         no [TODO]/[FIXME] without implementation · Logger initialized · page objects extend BasePage

❌ FAIL (auto-regenerate): any file < minimum line count · compilation errors
...
> **Purpose:** Prevent common issues in generated step definitions — permanent fixes and prevention guide.
...
**✅ CORRECT (Use `.filter()` method instead):**
...
**✅ CORRECT (Use single quotes inside selector strings):**
...
**✅ CORRECT (Comprehensive, multiple selectors):**
...
**❌ WRONG (`count() > 0` is ~10x slower and doesn't check visibility):**
...
- Login/Logout actions → Use `@When("user clicks on logout")` from LoginSteps
- Authentication → Use existing login methods
- Navigation → Use existing navigation steps
...
1. ✅ **ID Attribute** (Highest priority, most stable)
...
3. ✅ **Data-testid / Accessibility Attributes**
...
4. ⚠️ **Label Text** (Use as fallback)

   ```java
   page.getByLabel("Username")
   ```

5. ❌ **Avoid:** Generic selectors, XPath with indices

   ```java
   page.locator("input")                // BAD — too generic
   page.locator("//div[1]/input[2]")    // BAD — fragile
   ```
...
### Strategy 1: Context/Section Identifiers ⭐ RECOMMENDED
...
### Strategy 2: Unique Attributes ⭐ BEST FOR TEST AUTOMATION
...
### Strategy 3: Accessible ARIA Attributes ⭐
...
### Strategy 4: Position/Index ⚠️ FALLBACK ONLY

```java
// ⚠️ Only when order is guaranteed stable
...
│
├─→ Different sections/containers? → Use context locator (Strategy 1) ⭐
├─→ Have data-testid or unique IDs? → Use unique attributes (Strategy 2) ⭐
├─→ Different aria-labels? → Use ARIA (Strategy 3) ⭐
├─→ Position predictable & stable? → Use index with validation (Strategy 4) ⚠️
└─→ None of the above? → Ask devs to add unique identifiers
```
...
1. ✅ **Compilation** — Zero errors with `mvn clean compile`
2. ✅ **No Duplicates** — Zero duplicate step definition warnings
3. ✅ **Selector Validity** — All Playwright Java compatible
4. ✅ **Best Practices** — `.first().isVisible()` not `count() > 0`
5. ✅ **Assertion Messages** — All assertions have descriptive messages
6. ✅ **Logging** — Important steps logged

### Code Review Red Flags

- 🚫 Any `:has-text()` in selector strings
- 🚫 Double quotes inside double-quoted strings
- 🚫 Generic selectors like `page.locator("button")`
- 🚫 `count() > 0` for single element visibility
- 🚫 Assertions without messages
- 🚫 Duplicate step definition methods
...
public void clickElement() {
    System.out.println("📍 Step: User clicks on {element}");
...
public void enterText(String value) {
    System.out.println("📍 Step: Entering text: " + value);
...
public void verifyElementVisible() {
    System.out.println("📍 Step: Verifying {element} is visible");
    Locator element = page.locator("#elementId")
...
public void verifyErrorMessage() {
    System.out.println("📍 Step: Verifying error message");
    Locator error = page.locator(
        ".error, .alert-danger, [role='alert'], " +
        "[class*='error'], .invalid-feedback, [class*='invalid']"
    ).first();
...
# 🎯 Part 3: Smart Element Handler Reference
...
```java
// ❌ Which Add button? Fragile if order changes
...
### `clickButtonInContext()` ⭐ MOST RECOMMENDED
...
### `clickButtonSmart()` ⭐ PRODUCTION READY

**Priority order:** ID → data-testid → name → aria-label → text content
...
### `clickNthButton()` ⚠️ USE WITH CAUTION
...
### `clickFirstVisibleButton()` ⭐ RECOMMENDED
...
### `clickButtonNearElement()` ⭐ ADJACENT ELEMENTS
...
### `validateButtonState()` — Pre-condition checks
...
### `clickButtonWithRetry()` ⭐ FLAKY ELEMENTS
...
│
├─→ Button in specific section/container? → clickButtonInContext() ⭐
├─→ Button has unique ID or data-testid? → clickButtonSmart() ⭐
├─→ Button in table row? → clickButtonInRow() ⭐
├─→ Only one button visible at a time? → clickFirstVisibleButton() ⭐
├─→ Button near specific element? → clickButtonNearElement() ⭐
├─→ Button position is stable? → clickNthButton() ⚠️
└─→ Button may be slow to load? → clickButtonWithRetry() ⭐
```
...
## 🔧 SmartElementHandler Utility Class Implementation
...
public class SmartElementHandler {
    ...
    public static void clickButtonSmart(Page page, String buttonId, String buttonText) {
        ...
        if (button.count() == 0) {
            Assert.fail("Button not found — ID: " + buttonId + " / text: " + buttonText);
        }
    ...
    public static void clickButtonWithRetry(Page page, String buttonText, int maxRetries) {
        for (int i = 0; i < maxRetries; i++) {
            try {
                ...
                System.out.println("⚠️ Retry " + (i+1) + "/" + maxRetries + " for button: " + buttonText);
            }
        }
    }
}
...
|---|---|
| ❌ `quick-start.bat` — Windows only | ✅ `npm start` — Windows, macOS, Linux |
| ❌ `record-pw.ps1` — PowerShell only | ✅ Single source: `automation-cli.js` |
| ❌ 3+ script files to maintain | ✅ Professional, industry-standard |
...
```bash
npm start        # Shows interactive menu — press X to exit
...
```bash
npm run record   # Record actions → generates Page, Feature, Steps files
